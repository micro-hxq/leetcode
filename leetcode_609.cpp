...
public:
    vector<vector<string>> findDuplicate(vector<string>& paths) {
        ...
        for(const auto& str : paths)
        {
            ...
            string path, file, filename, content;
            input >> path;
            while(input >> file)
            {
                auto pospl = file.find_first_of('(');
                auto pospr = file.find_last_of(')');
                filename = file.substr(0, pospl);
                content = file.substr(pospl + 1, pospr - pospl - 1);
                aux[content].push_back(path + '/' + filename);
            }
        }
        for(const auto& files : aux)
        {
...
int main(int argc, char const *argv[])
{
    ...
    vector<string> paths = {"root/a 1.txt(abcd) 2.txt(efgh)", "root/c 3.txt(abcd)",
                            "root/c/d 4.txt(efgh)", "root 4.txt(efgh)"};
    auto res = sln.findDuplicate(paths);
    vector<vector<string>> ans = {
